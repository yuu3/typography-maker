...
export const useBody = () => {
  ...
  const onChangeBody = (value: string, id: string, target: "fontSize" | "fontWeight" | "lineHeight" | "letterSpacing") => {
  ...
  const addBody = () => {
    const id = generateId()
    const newBody = { id, fontSize: 16, fontWeight: 400, lineHeight: 1.5, letterSpacing: 0 }
