...
import { defaultFontSize, defaultScale } from "@/constants";
...
export const useFontScale = () => {
  ...
  useEffect(() => {
    for (const heading of headings) {
      const fontScale = fontScales.find(
        (fontScale) => fontScale.id === heading.tag
      );
    ...
    for (const body of bodies) {
      const fontScale = fontScales.find(
        (fontScale) => fontScale.id === body.id
      );

      if (fontScale)
        onChangeBody(fontScale.value.toString(), body.id, "fontSize");
    }
    for (const label of labels) {
      const fontScale = fontScales.find(
        (fontScale) => fontScale.id === label.id
      );

      if (fontScale)
        onChangeLabel(fontScale.value.toString(), label.id, "fontSize");
    }
  }, [fontScales]);
